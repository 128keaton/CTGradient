...
- (void)_commonInit;
- (void)setGradientBlendingMode:(CTGradientBlendingMode)mode;
...
- (id)init
  ...
  if (self != nil)
	{
	[self _commonInit];
	[self setGradientBlendingMode:CTLinearBlendingMode];
	}
  return self;
  }
...
- (id)copyWithZone:(NSZone *)zone
  ...
  [copy setGradientBlendingMode:blendingMode];
...
- (id)initWithCoder:(NSCoder *)coder
  ...
  [self setGradientBlendingMode:[coder decodeIntegerForKey:@"CTGradientBlendingMode"]];
  NSUInteger count = [coder decodeIntegerForKey:@"CTGradientElementCount"];
...
+ (id)rainbowGradient
  ...
  [newInstance setGradientBlendingMode:CTChromaticBlendingMode];
...
- (CTGradient *)gradientWithBlendingMode:(CTGradientBlendingMode)mode
  ...
  [newGradient setGradientBlendingMode:mode];
...
- (void)fillRect:(NSRect)rect angle:(CGFloat)angle
  ...
  if(angle == 0)		//screw the calculations - we know the answer
  	...
  	CGFloat rangle = angle * pi/180;	//convert the angle to radians
